...
public class RestTestComplexPOJO3 {
	...
	public void test() throws Exception {
		...
		Body body2 = new Body("raw", "{\"data\": \"123\"}");
		HeaderInner header2 = new HeaderInner("Content-Type", "application/json");
		ArrayList<HeaderInner> headerInner2 = new ArrayList<>();
		headerInner2.add(header);

		RequestRequest request2 = new RequestRequest("https://postman-echo.com/post", "POST", headerInner, body,
				"This is a sample POST Request");

		ItemsdfRequest itemInner2 = new ItemsdfRequest("This is a folder", request2);

		ArrayList<ItemsdfRequest> itemInnerObj2 = new ArrayList<>();
		itemInnerObj2.add(itemInner);
		ItemmRequest item2 = new ItemmRequest("gobba2", itemInnerObj2);
		...
		String uid = given().body(collectionRoot).when().post("collections").then().log().all().
		...
		String actualJson = obj.writeValueAsString(deserialized);
		 ...
		 ArrayList<String> requestUrls= new ArrayList<>();
			 ...
			 */
		 	...
		 	for(ItemmRequest itm:itemObj) {
			 for(ItemsdfRequest ii:itemInnerObj) {
			 
			  requestUrls.add(ii.getRequest().getUrl());
			  
			 System.out.println("Request "+ii.getRequest().getUrl());
			  }
		 	}
		...
		for(ItemsdfResponse sdf: iit.getItem()){
			...
			System.out.println("Response "+sdf.getRequest().getUrl().getRaw());
